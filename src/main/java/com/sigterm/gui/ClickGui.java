...
public class ClickGui extends Screen {
    ...
    private static final Set<String> expanded = new HashSet<>(); // expanded module settings
    static {
        int x = 6;
    ...
    public void render(GuiGraphics g, int mx, int my, float delta) {
        ...
        boolean gbh = mx>=gbx && mx<gbx+gbw && my>=2 && my<16;
        g.fill(gbx, 2, gbx+gbw, 16, gbh ? 0xFF5533AA : 0xFF332277);
    ...
    private void drawPanel(GuiGraphics g, Category cat, int mx, int my) {
        ...
        for (Module m : mods) {
            ...
            g.fill(x+2, cy, x+PW-2, cy+MH-1, rowBg);
            if (on) g.fill(x+2, cy, x+4, cy+MH-1, 0xFF00DD66);

            // Expand arrow if has settings
            if (!m.getSettings().isEmpty()) {
                g.drawString(font, Component.literal(exp ? "\u25bc" : "\u25b6"), x+6, cy+3, 0xFF555577);
            }

            int tc = binding ? 0xFFFFFF44 : (on ? 0xFF44FF88 : (hover ? 0xFFDDDDEE : 0xFF777799));
            g.drawString(font, Component.literal(m.name), x + (m.getSettings().isEmpty() ? 7 : 16), cy+3, tc);
            ...
            if (hover && !binding) { tooltip = m.description + "  |  R-click=rebind  Mid=expand"; tooltipX = mx; tooltipY = my; }
            cy += MH;
            ...
            if (exp) {
                for (Setting s : m.getSettings()) {
                    ...
                    g.drawString(font, Component.literal(val), x+PW-vw-8, cy+2, sHover ? 0xFFFFFFFF : 0xFFAAAACC);
                    cy += SH;
                }
            }
        }
        ...
        for (Category cat : Category.values()) {
            ...
            for (Module m : ModuleManager.INSTANCE.getByCategory(cat)) {
                ...
                if (mx>=xy[0]+2 && mx<xy[0]+PW-2 && my>=cy && my<cy+MH) {
                    ...
                    if (btn == 2) { // middle click = expand/collapse settings
                        if (!m.getSettings().isEmpty()) {
                ...
                if (exp) {
                    for (Setting s : m.getSettings()) {
                        if (mx>=xy[0]+4 && mx<xy[0]+PW-4 && my>=cy && my<cy+SH) {
                            if (btn == 0) { s.increment(); SigTermConfig.save(); return true; }
                            if (btn == 1) { s.decrement(); SigTermConfig.save(); return true; }
                        }
                        cy += SH;
                    }
                }
