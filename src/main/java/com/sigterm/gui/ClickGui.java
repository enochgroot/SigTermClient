...
public class ClickGui extends Screen {
    ...
    private static int getButton(MouseButtonEvent ctx) {
        try { return ctx.buttonInfo().button(); }
            ...
            try {
                var m = ctx.getClass().getMethod("button");
    ...
    public void render(GuiGraphics g, int mx, int my, float delta) {
        ...
        String guiKeyLabel = listeningForGuiKey ? "Menu: [PRESS KEY...]"
            : "Menu: [" + getKeyName(ModuleManager.INSTANCE.getGuiKeyBind()) + "]";
        ...
        g.drawString(font, Component.literal(active + " active"), width - font.width(active + " active") - 4, 5, 0xBBFFFFFF);
        ...
        if (listeningModule != null) {
            String msg = "Press key for " + listeningModule.name + "  (ESC=cancel DEL=unbind)";
            int msgW = font.width(msg);
            g.fill(width/2 - msgW/2 - 6, height - 30, width/2 + msgW/2 + 6, height - 14, 0xEE1A1A30);
            g.fill(width/2 - msgW/2 - 6, height - 30, width/2 + msgW/2 + 6, height - 28, 0xFFFFAA00);
            g.drawString(font, Component.literal(msg), width/2 - msgW/2, height - 26, 0xFFFFFF44);
        }
    ...
    private void drawPanel(GuiGraphics g, Category cat, int[] xy, boolean col, List<Module> mods, int mx, int my) {
        ...
        for (int i = 0; i < mods.size(); i++) {
            ...
            int rowBg = binding ? 0xFF2A1A3A : (on ? (hover ? 0xFF1A3A2A : 0xFF0F2A1A) : (hover ? 0xFF1E1E3A : 0xFF141428));
            g.fill(x+2, mmy, x+PW-2, mmy+MH-1, rowBg);
            if (on) g.fill(x+2, mmy, x+4, mmy+MH-1, 0xFF00DD66);
            int tc = binding ? 0xFFFFFF44 : (on ? 0xFF44FF88 : (hover ? 0xFFDDDDEE : 0xFF777799));
            g.drawString(font, Component.literal(m.name), x+7, mmy+3, tc);
            String kl = binding ? "..." : getKeyName(m.getKeyBind());
            int kw = font.width(kl) + 6;
            g.fill(x+PW-kw-4, mmy+1, x+PW-3, mmy+MH-2, binding ? 0xFF7744FF : 0x44FFFFFF);
            g.drawString(font, Component.literal(kl), x+PW-kw-1, mmy+3, binding ? 0xFFFFFF44 : 0xFF888899);
            if (hover && !binding) { tooltip = m.description + "  |  R-click = rebind"; tooltipX = mx; tooltipY = my; }
        }
    ...
    public boolean mouseClicked(MouseButtonEvent ctx, boolean doubleClick) {
        ...
        String gkl = "Menu: [" + getKeyName(ModuleManager.INSTANCE.getGuiKeyBind()) + "]";
            ...
            if (!col) {
                ...
                for (int i = 0; i < mods.size(); i++) {
                    ...
                    if (mx>=xy[0]+2 && mx<xy[0]+PW-2 && my>=mmy && my<mmy+MH) {
                        if (btn == 0) { mods.get(i).toggle(); SigTermConfig.save(); return true; }
                        if (btn == 1) { listeningModule = mods.get(i); return true; }
                    }
                }
            }
        }
    ...
    public boolean mouseDragged(MouseButtonEvent ctx, double dx, double dy) {
        if (dragging != null) {
            ...
            xy[0] = (int)(ctx.x() - dragOffX); xy[1] = (int)(ctx.y() - dragOffY);
            return true;
        }
    ...
    public boolean keyPressed(KeyEvent ctx) {
        ...
        if (listeningForGuiKey) {
            if (key == GLFW.GLFW_KEY_ESCAPE) { listeningForGuiKey = false; return true; }
            if (key == GLFW.GLFW_KEY_DELETE || key == GLFW.GLFW_KEY_BACKSPACE) ModuleManager.INSTANCE.setGuiKeyBind(0);
            else ModuleManager.INSTANCE.setGuiKeyBind(key);
            listeningForGuiKey = false; SigTermConfig.save(); return true;
        }
        if (listeningModule != null) {
            if (key == GLFW.GLFW_KEY_ESCAPE) { listeningModule = null; return true; }
            if (key == GLFW.GLFW_KEY_DELETE || key == GLFW.GLFW_KEY_BACKSPACE) listeningModule.setKeyBind(0);
            else listeningModule.setKeyBind(key);
            listeningModule = null; SigTermConfig.save(); return true;
        }
